...
function assign_function(str)
{
	...
	if (str == "") {
        ...
        xmlhttp.open("GET","assign_officer_rev.php?"+str,true);
        xmlhttp.send();
		...
		if(!empty($_POST['add-id'])&&!empty($_POST['add-pass'])&&!empty($_POST['add-name'])&&!empty($_POST['add-designation']))
		{
			...
			else
			{
				...
				else
				{
					...
					$query = "insert into review_officer (rev_off_id,passwd,name,designation) values ('$id','$passwd','$name','$designation')";
...
<!--option no-1 Add an officer-->
 				<div id="add" class="tab-pane fade in active" >
 					<h3>Create New Review Officer</h3>
...
<!--option no-3 assign an officer's details -->
				<div id="assign" class="tab-pane fade">
					<h3>Assign Reporting Officer</h3><button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#viewrepoff">View Review Officers</button>

	<?php


		$query1 = "select * from reporting_officer order by sno";
...
<?php
	...
	{	
		...
		$query2 = "select * from review_officer";
...
?>
				...
				<td>
					...
					<option value="">select</option>
						...
						{	
							$name_rev_off = $row2['name'];
							$designation_rev_off = $row2['designation'];
							$review_id = $row2['rev_off_id'];

							$query3 = "select count(*) as count_rep from reporting_officer where rev_off_id = '$review_id'";
							...
							$count_rep=$row3['count_rep'] ;
					?>
  
					<option value="<?php echo "rev_off_id=".$review_id."&rep_off_id=".$rep_off_id ; ?>" ><?=$count_rep," ",$name_rev_off?></option>
...
<!-- Model for view the total reporting officers with number of assigned reporting_officers under them-->
<div class="modal fade" id="viewrepoff" role="dialog">
      							...
      							<div class="modal-content" style="color: black">
        							<div class="modal-header">
          								...
          								<table class="table">
											<thead>
    											<tr style="font-wieght:bold ">
												<th >REview Officer ID</th>
												<th>Officer Name</th>
												<th>Designation</th>
												<th>Review for Rep.Officers(numbers)</th>
												</tr>
    										</thead>
          								</h4>
        							</div>
        				...
        				<div class="modal-body">
          				<?php
          						$query4 = "select * from review_officer";
						...
						{	
							$name_rev_off = $row4['name'];
							$designation_rev_off = $row4['designation'];
							$review_id = $row4['rev_off_id'];

							$query5 = "select count(*) as count_rep from reporting_officer where rev_off_id = '$review_id'";
							...
							$count_rep=$row5['count_rep'] ;
					?>
        					<tbody>
								<tr>
									<td><?=$review_id?></td>
									<td><?=$name_rev_off?></td>
									<td><?=$designation_rev_off?></td>
									<td><?=$count_rep?></td>
